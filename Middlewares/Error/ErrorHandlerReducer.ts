...
export const EXISTING_USER_FOUND_IN_DATABASE = async (user_registered_email: string, user_auth_type_specified: AuthTypeDeclared, authorities_provided_by_role: RolesSpecified) => {
    const exisiting_user_found = (authorities_provided_by_role === RolesSpecified.ADMIN_DESC) ? await admin_detailed_structure_description.findOne({ admin_userEmail: user_registered_email }) : await user_detailed_description.findOne({ registered_user_email: user_registered_email })

    return (user_auth_type_specified === AuthTypeDeclared.USER_REGISTRATION && exisiting_user_found) ||
        (user_auth_type_specified === AuthTypeDeclared.USER_REGISTRATION && exisiting_user_found)
